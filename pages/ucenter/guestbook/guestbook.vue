<template>
	<view class="page">
		<unicloud-db ref="udb" class="content" v-slot:default="{data,pagination,hasMore, loading, error}"
			collection="guestbook" :where="udbWhere" field="userName,text,_id,state">
			<view v-if="error">{{error.message}}</view>
			<view v-else>
				<view v-for="(item,index) in data" :key="index" class="item">
					<view class="main">
						<!-- <cloud-image :src="item.user_id[0].avatar_file.url"></cloud-image> -->
						<image class="userImg" width="60rpx" height="60rpx" src="/static/guest_logo.png"></image>
						<view>
		...
		<view class="submit-box">
			...
			<image class="userImg" src="/static/mine.png" mode="widthFix"/>

		...
		<!-- <view class="warp"> -->
			<swiper :current="tabIndex" class="swiper-box" style="flex: 1;" :duration="300" @change="ontabchange">
				<swiper-item class="swiper-item">
					<uni-section  title="我的待验收记录" style="margin: 0;" type="line"></uni-section>
					...
					<scroll-view scroll-y="true" class="scroll-Y">
						...
						<view v-for="(item,index) in myData" @longpress="longtap(item)"  @touchend="touchEnd" @touchmove="touchMove">
						  <uni-card :title="'序号： '+(index+1)" extra=">" @click="toDetail(item)">
							  <uni-list>
								<uni-list-item title="检修编号" :note="item.techNo"></uni-list-item>
								<uni-list-item title="检修记录" :note="item.techName"></uni-list-item>
			     ...
			     <swiper-item class="swiper-item">
					 <uni-section  title="全部验收记录" style="margin: 0;" type="line"></uni-section>
				...
				<swiper-item class="swiper-item">		
				<uni-section  title="待验收的检修记录" style="margin: 0;" type="line"></uni-section>
				...
				<swiper-item class="swiper-item">
					<uni-section  title="已验收的检修记录" style="margin: 0;" type="line"></uni-section>
						<scroll-view scroll-y="true" class="scroll-Y">
	   ...
	   async onLoad(e) {
        ...
        methods: {
			...
			openCard(item,editFlag){
				...
				uni.navigateTo({
					url: '../repair/equipmentCard?planNo=' + item.planNo+'&lineNo='+item.lineNo+'&seqNo='+item.seqNo+'&techId='+item.techId+'&editFlag='+editFlag,
					success: res => {},
					fail: () => {},
					complete: () => {}
				});
						...
						if (res.confirm) {
							...
							uni.showModal({
								...
								success: function (res) {
									if (res.confirm) {
										item.uid = userNum;
										item.acceptNote = res.content;
										item.acceptOpinion = "合格";
			...
			async getData(loading){
				...
				var records =await this.$requestMaintPlanLine({
							url:'/getcheckpointequiprecord/'+woNo,
							loading:loading
						});
				...
				for(var i in records){
					if(records[i].finishAccepter!=records[i].allAccepter){
						...
						// 待验收
						if(records[i].indexOf(userNum)!=-1){
